...
export default function Demo_MainNav_Tabbed_VerticalTabs() {
  ...
  return (
    ...
    >
      <Canvas level={1}>
        <div className="mb-5">
          <h1 className="text-[16px] font-semibold text-gray-900">{activeBodyTab}</h1>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="border border-gray-200 rounded-lg p-6 h-40 flex items-center justify-center">
        <span className="text-[13px] text-gray-400">{activeTab} visualization</span>
      </div>
      <div className="grid grid-cols-3 gap-4">
        {['Metric 1', 'Metric 2', 'Metric 3'].map(label => (
          <div key={label} className="border border-gray-200 rounded-lg p-4">
            <p className="text-[12px] font-medium text-gray-500 mb-2">{label}</p>
            <div className="h-6 w-20 bg-gray-100 rounded" />
          </div>
        ))}
      </div>
      <div className="border border-gray-200 rounded-lg p-4 h-48 flex items-center justify-center">
        <span className="text-[13px] text-gray-400">Chart or detailed breakdown</span>
      </div>
    </div>
  )
}
